        ...
        with ui.row().classes('w-full flex-grow gap-0').style('min-height: 0; overflow: hidden;'):
            ...
            with ui.column().classes(
                ...
                with ui.row().classes('w-full gap-2'):
                    ...
                    ).classes('flex-grow font-bold').props('dense unelevated no-caps').style(
                        'background-color: var(--btn-start) !important; color: white !important;'
                    )
                    stop_btn = ui.button(
                        'STOP',
                        on_click=lambda: _stop(controller, state, stop_btn),
                    ).classes('flex-grow font-bold').props('dense unelevated no-caps').style(
                        'background-color: var(--btn-stop) !important; color: white !important;'
                    )
                    stop_btn.disable()
